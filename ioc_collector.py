...
class IOCCollector:
    ...
    def get_user_iocs(self, user):
        iocs = [
            ioc["indicator"]
            for ioc in self.otx.get_all_indicators(
                author_name=user,
                indicator_types=[IndicatorTypes.DOMAIN, IndicatorTypes.HOSTNAME],
            )
        ]
        return iocs
...
class GitHubUpdater:
    ...
    def update_iocs(self):
        ...
        iocs_b64 = base64.b64encode(
            "\n".join(self.ioc_collector.get_all_iocs()).encode()
        ).decode()

        headers = {"Authorization": f"token {self.cfg['github_key']}"}
        payload = {
            "message": f"IOCs updated on {datetime.now()}",
            "content": iocs_b64,
            "sha": self._get_current_sha(),
        }
        r = requests.put(
            self.cfg["github_update_path"], data=json.dumps(payload), headers=headers
        )
        print(r.json())
